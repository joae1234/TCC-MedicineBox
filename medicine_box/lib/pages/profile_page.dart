...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Profile? _profile;
  bool _loading = true;
  List<Profile> _relatedProfiles =
      []; // Lista de pacientes ou cuidadores associados
  ...
  Future<void> _load() async {
    try {
      final p =
          await _profileSvc
              .getOwnProfile(); // Carrega o perfil do usuário logado
      if (!mounted) return;
      setState(() {
        _profile = p;
        _loading = false;
  ...
  Future<void> _removeRelation(String relatedId) async {
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('Erro ao excluir vínculo: $e')));
    }
  }
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    ListTile(
                      ...
                      subtitle: Text(
                        _profile!.role.isEmpty
                            ? '-'
                            : roleMapping[_profile!.role] ?? '-',
                      ),
                    ),
                    const Divider(),
                    ListTile(
